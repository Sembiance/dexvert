...
exports.args = (state, p, inPath=state.input.absolute) => (["-r", "-y", inPath]);
...
exports.post = (state, p, cb) =>
{
	tiptoe(
		...
		function convertImagesToPDF(rtfRaw)
		{
			...
			this.data.bitmapFilePaths.parallelForEach((bitmapFilePath, subcb) =>
			{
				if(bitmapFilePath.endsWith(".shg"))
					p.util.program.run("deark", {args : p.program.deark.args(state, p, bitmapFilePath, state.helpdecoTmpDirPath, path.basename(bitmapFilePath))})(state, p, subcb);
				else
					setImmediate(subcb);
			}, this);
		},
		function convertToPNG()
		{
			...
			{
				if(bitmapFilePath.endsWith(".shg"))
					p.util.program.run("convert", {args : p.program.convert.args(state, p, `${bitmapFilePath}.000.bmp`, `${bitmapFilePath}.png`)})(state, p, subcb);
				else
					p.util.program.run("convert", {args : p.program.convert.args(state, p, bitmapFilePath, `${bitmapFilePath}.png`)})(state, p, subcb);
			}, this);
		},
		function loadImages()
		{
		...
		function convertRTFToPDF()
		{
			...
			p.util.program.run("unoconv", {args : p.program.unoconv.args(state, p, this.data.rtfFilePath)})(state, p, this);
		},
		function cleanup()
		{
