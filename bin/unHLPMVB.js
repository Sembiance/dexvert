...
[
	...
	{prefix : "\\pard ", regex : /\\{ew[lrc] .*!(?<filename>[^}\\]+)\\}/g}						// multimedia viewer MVBMP2 import
];
...
if(rtfFilePath && (await Deno.stat(rtfFilePath))?.size)
{
	...
	{
		for(const {prefix : pictPrefix, regex : imageRefRegex} of IMAGE_REF_REGEXES)
		{
			rtfLine = await rtfLine.replaceAsync(imageRefRegex, async (m, filename) =>	// eslint-disable-line no-param-reassign
			{
				const rtfPNG = hex => `${pictPrefix || ""}{\\pict\\pngblip\n${hex}}`;
