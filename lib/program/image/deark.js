...
exports.bin = () => "deark";
exports.args = state => (["-opt", "char:output=image", "-od", state.output.dirPath, "-o", state.input.name, state.input.filePath]);
		...
		tiptoe(
			...
			function convertOutputFiles(outputFilePaths)
			{
				...
				outputFilePaths.parallelForEach((outputFilePath, subcb) =>
				{
					...
					if(ext===".qtif")
						...
						p.util.program.run(p.program.recoil2png, {args : ["-o", path.join(state.output.absolute, `${path.basename(outputFilePath, ext)}.png`), path.join(state.output.dirPath, path.basename(outputFilePath))]})(state, p, subcb);
					else
			...
			function renameIfNeeded(outputFilePaths)
			{
				...
				if(state.dearkJoinFrames)
				{
					...
					p.util.program.run(p.program.convert, {args : ["-delay", "12", "-loop", "0", "-dispose", "previous", ...outputFilePaths, ...p.program.convert.STRIP_ARGS, this.data.GIFFilePath]})(state, p, this);
				}
				else
				{
			...
			{
				p.util.program.run(p.program.ffmpeg, {args : ["-i", this.data.GIFFilePath, "-c:v", "libx264", "-crf", "1", "-preset", "slow", path.join(state.output.absolute, `${state.input.name}.mp4`)]})(state, p, this);
			},
