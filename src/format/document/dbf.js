...
export class dbf extends Format
{
	...
	forbidExtMatch = true;
	magic          = [
		/^(?:FoxBase\+?\/?)?dBase .*DBF/, "dBASE Database", /^(dBase|xBase) .*DBF/, "Table MS Visual FoxPro", "FoxPro with memo DBF", "Visual FoxPro", "dBase Datendatei", "Form MS Visual FoxPro 7", "FoxPro Screen (generic)",
	...
	idMeta         = ({macFileType, macFileCreator}) => (["F+DB", "F+FR", "F+LB", "F+RP", "FDBC", "FMNX", "FPJX", "FSCX"].includes(macFileType) && ["FOX+", "FOXX"].includes(macFileCreator)) || ([".DBF", ".Dxx"].includes(macFileType) && macFileCreator==="ACT!");
