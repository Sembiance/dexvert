...
exports.process = function process(_inputFilePath, _outputDirPath, {verbose=0, brute=false, keepGoing=false, alwaysBrute=false, brutePrograms=false, midiFont=null, useTmpOutputDir=null, dontTransform=false}, cb)
{
	...
	tiptoe(
		...
		{
			console.log(`cleaning up ${tmpOutputDirPath}`);
		...
		function trimInputFile()
		{
			...
			if(state.verbose>=2)
				XU.log`Transforming file with trimGarbage and stripGarbage`;
			
			processWithTransform("trimGarbage", state, {verbose, brute, keepGoing, alwaysBrute, brutePrograms, midiFont, useTmpOutputDir}, this.parallel());
			processWithTransform("stripGarbage", state, {verbose, brute, keepGoing, alwaysBrute, brutePrograms, midiFont, useTmpOutputDir}, this.parallel());
		},
		function returnResult(err, trimmedState, strippedState)
		{
			let r = state;
			if(strippedState && strippedState?.id?.formatid!=="emptyFile")
				r = strippedState;
			if(trimmedState && trimmedState?.id?.formatid!=="emptyFile")
				r = trimmedState;
...
function processWithTransform(transformType, state, processOptions, cb)
{
	const transformedFilePath = fileUtil.generateTempFilePath(undefined, state.input.ext);
	...
	tiptoe(
		...
		function performProcess(transformedFileExists)
		{
			...
			exports.process(transformedFilePath, state.output.absolute, {dontTransform : true, ...processOptions}, this);
		},
		function cleanup(transformedState)
		{
			if(transformedState?.id)
			{
				const transformedMeta = transformedState.input.meta;
				transformedState.input = XU.clone(state.input);
				transformedState.input.meta = transformedMeta;
				this.data.transformedState = transformedState;
			}

			fileUtil.unlink(transformedFilePath, this);
		},
		function returnState()
		{
			this(undefined, this.data.transformedState);
		},
		cb
	);
}
