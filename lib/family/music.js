...
[
	(state, p) =>
	{
		...
		if(!(p.format.converterExclude || []).includes("*"))
			state.converters.push(...DEFAULT_CONVERTERS.filter(d => !(p.format.converterExclude || []).includes(d) && !state.converters.some(c => c.program===d) && !p.program[d].meta.bruteUnsafe).map(d => ({program : d})));
	...
	(state.output.files || []).slice().parallelForEach((outSubPath, subcb) =>
	{
		...
		tiptoe(
			...
			function convertOrRemove()
			{
				...
				p.util.program.run("ffmpeg", {flags : {ffmpegExt : ".mp3"}, argsd : [outFilePath, path.join(path.dirname(outFilePath), `${path.basename(outFilePath, path.extname(outFilePath))}.mp3`)]})(state, p, this);
			},
