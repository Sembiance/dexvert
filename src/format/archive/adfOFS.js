...
export class adfOFS extends Format
{
	...
	fileSize      = 901_120;
	matchFileSize = true;
	magic         = ["Amiga Disk image File (OFS", "Amiga DOS disk", "Amiga OFS file system", "application/x-amiga-disk-format", /^Amiga (Fastdir|Inter) DOS disk/, /^fmt\/1361( |$)/];
	notes         = xu.trim`
	...
	converters =  ["unar[filenameEncoding:iso-8859-1]", "uaeunp", "unadf", "extract_adf[matchType:magic]", "extract_adf[hasExtMatch]"];	// unadf handles files better, but doesn't preserve all timestamps? unar preserves timestamps well.
}
