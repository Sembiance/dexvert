		...
		function loadFormats(familyFormatPaths)
		{
			...
			C.FAMILIES.forEach((FAMILY, i) =>
			{
				familyFormatPaths[i].forEach(familyFormatPath =>
				{
					...
					const unknownMetaKeys = Object.keys(format.meta).subtractAll(["name", "website", "ext", "safeExt", "mimeType", "forbidExtMatch", "unsupported", "magic", "forbiddenMagic", "filename", "priority", "weakMagic", "trustMagic",
						"notes", "bruteUnsafe", "filesize", "keepFilename", "symlinkUnsafe", "encoding", "confidenceAdjust", "forbidFilesizeMatch"]);
					if(unknownMetaKeys.length>0)
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has extra meta keys: ${unknownMetaKeys}`);

					const unknownFormatKeys = Object.keys(format).subtractAll(["meta", "steps", "post", "updateProcessed", "custom", "inputMeta", "preSteps", "postSteps", "converterPriorty", "converterExclude"]);
					...
					if((meta.ext || []).some(ext => ext.match(/[A-Z]/)))
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has an ext that is not lowercase!`);

					if(!meta.hasOwnProperty("name"))
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} does not have a name!`);
			...
			Object.forEach(require(path.join(C.FORMAT_DIR_PATH, "unsupported.js")).formats, (family, unsupportedFormats) =>
			{
				Object.forEach(unsupportedFormats, (formatid, format) =>
				{
					...
					if(format.custom)
					{
						formats[family][formatid].custom = format.custom;
						delete formats[family][formatid].meta.custom;
					}
				});
			});
