...
start = async () =>
{
	...
	({p} = await runUtil.run("python3", ["supervisor.py", `--workers=${C.IS_DEV_MACHINE ? 3 : 10}`, `--port=${C.GAMEEXTRACTOR_PORT}`], {detached : true, cwd : path.join(import.meta.dirname, "..", "..", "bin", "GameExtractorServer"), exitcb : async o => await exitHandler(o), stdoutcb : line => xlog.info`${line}`, stderrcb : line => xlog.warn`${line}`}));
