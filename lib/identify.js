...
exports.identify = function identify(state, {C, formats}, cb)
{
	...
	Object.forEach(formats, (formatFamily, formatTypes) =>
	{
		...
		Object.forEach(formatTypes, (formatid, format) =>
		{
			...
			const extMatch = ((!meta.unsupported || format.custom) || meta.magic) && (meta.ext || []).some(ext => state.input.base.toLowerCase().endsWith(ext));
			const filenameMatch = (meta.filename || []).some(mfn => C.flexMatch(state.input.base, mfn, true));
			const filesizeMatch = (meta.filesize || []).some(mfs => Array.force((typeof mfs==="function" ? mfs(state) : mfs)).filterEmpty().includes(inputFileSize));
			...
			if(extMatch && !meta.forbidExtMatch && (!meta.filesize || magicMatch || filesizeMatch))
			{
				const familyMatch = {...baseMatch, matchType : "ext", matchesMagic : magicMatch};
				if(meta.magic)
					familyMatch.hasMagic = meta.magic;
				familyMatches.ext.push(familyMatch);
			}
		...
		[["magic", 100], ["ext", 66], ["filename", 33], ["filesize", 20]].forEach(([matchType, startConfidence]) =>
		{
			...
			if(matchType==="ext")
				familyMatches[matchType].multiSort([m => ((m.hasMagic && !m.matchesMagic) ? 1 : 0)]);
