...
const argv = cmdUtil.cmdInit({
	cmdid   : "dexserver-dragonUnpacker",
	version : "1.0.0",
	desc    : "Runs and monitors the dragonUnPACKer daemon which is used to help identify and extract files",
	opts    :
	{
...
const exitHandler = async ({success, code, signal}) =>
{
	...
	xlog.error`dragonUnpacker server exited unexpectedly (success: ${success}) with code: ${code}, signal: ${signal}`;
	await start();
	xlog.info`dragonUnpacker server restarted successfully`;
};

start = async () =>
{
	xlog.info`Starting dragonUnpacker server...`;
	({p} = await runUtil.run(Program.binPath("dragonUnpackerServer"), [], {detached : true, exitcb : async o => await exitHandler(o), stdoutcb : line => xlog.info`${line}`, stderrcb : line => xlog.warn`${line}`}));
