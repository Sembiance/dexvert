...
tiptoe(
	...
	function getFormats(goodPNGPaths, formats)
	{
		...
		const recoilFormats = Object.entries(formats.image).filter(([, format]) => (format.converterPriorty || []).length===1 && (format.converterPriorty || []).includes("recoil2png") && format.meta.fileSize);
