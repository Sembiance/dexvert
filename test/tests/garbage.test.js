...
import {classifyImage} from "/mnt/compendium/DevLab/dexvert/src/classifyUtil.js";
...
//xlog.warn`garbage: ${(await GARBAGE.parallelMap(async v => await classifyImage(path.join(xu.dirname(import.meta), "garbage", v), "garbage", xlog))).map(v => v.toFixed(10))}`;
//xlog.warn`notGarbage: ${(await NOT_GARBAGE.parallelMap(async v => await classifyImage(path.join(xu.dirname(import.meta), "notGarbage", v), "garbage", xlog))).map(v => v.toFixed(10))}`;
