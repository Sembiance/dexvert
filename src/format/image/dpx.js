...
export class dpx extends Format
{
	...
	metaProvider = ["image"];
	converters   = ["convert", "ffmpeg[format:dpx_pipe][outType:png]", "wuimg[format:dpx]", "nconvert[format:dpx]"];	// iconvert also supports it but produces bad output, abydos claims support, but failed to get output
}
