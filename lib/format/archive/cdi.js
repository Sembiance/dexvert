...
exports.steps =
[
	(state, p) => p.util.wine.run({cmd : "IsoBuster/IsoBuster.exe", args : [`/ef:${p.util.wine.path(path.join(state.cwd, state.output.dirPath))}`, path.join(state.cwd, state.input.filePath).replaceAll("/", "\\"), "/c"], timeout : XU.MINUTE*10})
];
