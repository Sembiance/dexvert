...
function init(cb)
{
	tiptoe(
		...
		function findFormats(utilFilePaths, programFilePaths, familyFilePaths, formats)
		{
			...
			[["util", utilFilePaths], ["program", programFilePaths], ["family", familyFilePaths]].forEach(([type, filePaths]) => filePaths.forEach(filePath => { p[type][path.basename(filePath, ".js")] = require(filePath); }));
...
exports.process = function process(inputFilePath, outputDirPath, {tmpDirPath=os.tmpdir(), verbose=0, brute=false, keepGoing=false}, cb)
{
	const state =
	{
		...baseState,
