...
export class uade123 extends Program
{
	...
	flags   = {
		...
		maxDuration : "Maximum duration (in seconds) to allow the output file to be. Default: 15 minutes"
	};
	bin  = "uade123";
	args = async r => [...(r.flags.player ? ["-P", `/usr/share/uade/players/${r.flags.player}`] : []), "-t", r.flags.maxDuration || 900, "-e", "wav", "-f", await r.outFile("out.wav"), r.inFile()];
