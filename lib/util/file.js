	...
	return (state, p, cb) =>
	{
		...
		tiptoe(
			...
			function gatherOutputFileInfo(outputFilePaths)
			{
				...
				outputFilePaths.parallelForEach((outputFilePath, subcb) => fileUtil.areEqual(state.input.absolute, outputFilePath, subcb), this.parallel());
			},
			function removeEmptyOutputFiles(fileStats, equalOutputFiles)
			{
				const badOutputFiles = this.data.outputFilePaths.filter((outputFilePath, i) => fileStats[i].size===0 || equalOutputFiles[i]);
				...
				if(state.verbose>=4 && badOutputFiles.length>0)
					XU.log`file.findValidOutputFiles found ${badOutputFiles.length} bad output files. Deleting them...`;
