...
class Ms3dImporter():
    ...
    def read(self, blender_context, filepath):
        ...
        try:
            ...
            if self.options_verbose in Ms3dUi.VERBOSE_NORMAL:
                print(f"read - exception in try block\n  type: '{exc_type}'")
                print_exc()
                if self.report:
    ...
    def create_geometry(self, blender_context, ms3d_model):
        ...
        for ms3d_material_index, ms3d_material in enumerate(
            ...
            if ms3d_material.texture:
                node_i2 = nodes.new('ShaderNodeTexImage')
                node_i1 = nodes.new('ShaderNodeBsdfPrincipled')
                node_i0 = nodes.new('ShaderNodeOutputMaterial')
                node_i2.location = (0.0, 0.0)
                node_i1.location = (node_i2.location.x + node_i2.width + gap, 0.0)
                node_i0.location = (node_i1.location.x + node_i1.width + gap, 0.0)
                link_i2_0 = links.new(node_i2.outputs['Color'], node_i1.inputs['Base Color'])
                link_i1_0 = links.new(node_i1.outputs['BSDF'], node_i0.inputs['Surface'])
                node_i2.image = blender_image_diffuse
    ...
    def create_animation(self, blender_context, ms3d_model, blender_mesh_object):
        ...
        # setup scene
        blender_scene = blender_context.scene
        blender_scene.render.fps = int(ms3d_model.animation_fps)
        if ms3d_model.animation_fps:
            blender_scene.render.fps_base = int(blender_scene.render.fps /
                    ms3d_model.animation_fps)

        blender_scene.frame_start = 1
        blender_scene.frame_end = (ms3d_model.number_total_frames
                + blender_scene.frame_start) - 1
        blender_scene.frame_current = int(ms3d_model.current_time
                * ms3d_model.animation_fps)
