			...
			function convertOutputFiles(outputFilePaths)
			{
				...
				outputFilePaths.parallelForEach((outputFilePath, subcb) =>
				{
					...
					if(ext===".qtif")
						...
						p.util.program.run(p.program.convert, {args : [path.join(state.output.dirPath, path.basename(outputFilePath)), "-strip", path.join(state.output.dirPath, `${path.basename(outputFilePath, ext)}.png`)]})(state, p, subcb);
				}, this);
			},
