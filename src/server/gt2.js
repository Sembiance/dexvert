...
const argv = cmdUtil.cmdInit({
	cmdid   : "dexserver-gt2",
	version : "1.0.0",
	desc    : "Runs and monitors the gt2 daemon which is used to help identify files",
	opts    :
	{
...
const exitHandler = async ({success, code, signal}) =>
{
	...
	xlog.error`gt2 server exited unexpectedly (success: ${success}) with code: ${code}, signal: ${signal}`;
	await start();
	xlog.info`gt2 server restarted successfully`;
};

start = async () =>
{
	xlog.info`Starting gt2 server...`;
	({p} = await runUtil.run(path.join(Program.binPath("gt2"), "gt2.py"), [`--workers=${C.IS_DEV_MACHINE ? 3 : 10}`, `--port=${C.GT2_PORT}`, `--host=${C.GT2_HOST}`, "--request-timeout=10"], {detached : true, exitcb : async o => await exitHandler(o), stdoutcb : line => xlog.info`${line}`, stderrcb : line => xlog.warn`${line}`}));
	await xu.waitUntil(async () => (await xu.fetch(`http://${C.GT2_HOST}:${C.GT2_PORT}/status`, {silent : true}))?.trim()==="a-ok");
};
