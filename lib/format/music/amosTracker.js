...
[
	() => (state, p, cb) =>
	{
		tiptoe(
			...
			{
				p.util.program.run(p.program.xmp, {args : ["-o", path.join(state.output.dirPath, "outfile.wav"), this.data.modFilename]})(state, p, this);
			},
			function getMusicInfo()
			{
				p.util.program.run(p.program.modInfo, {args : ["--jsonOutput", path.join(state.cwd, this.data.modFilename)]})(state, p, this);
			},
