...
exports.post = (state, p, r, cb) => p.util.file.move(path.join(state.output.absolute, `outfile.${(r.flags.unoconvType || (state.id.family==="image" ? "png" : "pdf"))}`),
	path.join(state.output.absolute, `${state.input.name}.${(r.flags.unoconvType || (state.id.family==="image" ? "png" : "pdf"))}`))(state, p, cb);
