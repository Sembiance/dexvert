...
exports.steps =
[
	(state, p) => p.util.dos.run({state, p, bin : "TSCOMP.EXE", autoExec : [`TSCOMP.EXE -l ${state.input.filePath} > TSFILES.TXT`]}),
	(state, p) =>
	{
		...
		return p.util.dos.run({state, p, autoExec : tscompFilenames.map(fn => `TSCOMP.EXE -d ${state.input.filePath} ${state.output.dirPath}\\${fn}`)});
	}
];
