...
function performStep(errorLoc, state, p, step, cb)
{
	if(step.length===3)
	{
		console.error("step function is expecting 3 cbs, but should only be 2. Likely need to do () => fun instead of just 'fun'");
...
exports.serial = function serial(steps)
{
	...
	const errorLoc = new Error("util.flow.serial");
...
exports.parallel = function parallel(steps)
{
	...
	const errorLoc = new Error("util.flow.parallel");
