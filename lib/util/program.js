	...
	return (state, p, cb) =>
	{
		...
		tiptoe(
			...
			function executeBin()
			{
				...
				const runOptions = Object.assign({silent : state.verbose<=1, liveOutput : state.verbose>=3, timeout : XU.MINUTE*10, cwd : state.cwd}, (options.runOptions || (program.runOptions ? program.runOptions(state) : {})));
