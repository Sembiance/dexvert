...
exports.bin = () => "ffmpeg";
exports.args = (state, p, r, inPath=state.input.filePath, outPath=path.join(state.output.dirPath, `outfile${r.flags.ffmpegExt || (state.id.family==="image" ? ".png" : (state.id.family==="audio" ? ".flac" : ".mp4"))}`)) =>
{
	const ffmpegArgs = [];

	// Can run `ffmpeg -formats` for a list
	r.flags.ffmpegExt = r.flags.ffmpegExt || (state.id.family==="image" ? ".png" : (state.id.family==="audio" ? ".flac" : ".mp4"));
