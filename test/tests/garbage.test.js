...
import {classifyImage} from "../../src/tensorUtil.js";
...
const SAMPLE_FILE_PATH = path.join(xu.dirname(import.meta), "../../tensor/garbage/train");

Deno.test("garbage", async () => assertEquals([1, 1, 3.1692634627766836e-12, 1, 1, 1, 1], await GARBAGE.parallelMap(async v => await classifyImage(path.join(SAMPLE_FILE_PATH, "garbage", v), "garbage"))));	// eslint-disable-line unicorn/numeric-separators-style

Deno.test("notGarbage", async () => assert((await NOT_GARBAGE.parallelMap(async v => await classifyImage(path.join(SAMPLE_FILE_PATH, "notGarbage", v), "garbage"))).every(v => v===0)));
