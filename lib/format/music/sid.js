...
exports.inputMeta = (state, p, cb) =>
{
	tiptoe(
		function getSIDInfo()
		{
			runUtil.run("sidplay2", ["-w/dev/null", "-t1", state.input.filePath], {silent : state.verbose<=1, liveOutput : state.verbose>=3, timeout : XU.MINUTE*5, cwd : state.cwd}, this.parallel());
...
exports.steps =
[
	(state0, p0) => p0.util.flow.serial([].pushSequence(1, (state0.input.meta.sid.subSongCount || 1)).map(subSong => state =>
	{
		state.sidSubTune = subSong;
		state.sidSongLength = (state.input.meta.sid.songLengths || [])[(subSong)-1] || "3:0";
		return {program : "sidplay2"};
	})),
