"use strict";
/* eslint-disable prefer-named-capture-group */
const XU = require("@sembiance/xu"),
	path = require("path"),
	C = require(path.join(__dirname, "..", "..", "C.js"));

// txtByFilename handles files with specific filenames that are likely text but have non-ascii characters which requires loosened magic match of A.GENERIC_MAGIC which includes "data"
exports.meta =
{
	name      : "Text File",
	website   : "http://fileformats.archiveteam.org/wiki/Text",
	magic     : [...C.TEXT_MAGIC, ...C.GENERIC_MAGIC],
	weakMagic : true,
	priority  : C.PRIORITY.VERYLOW,
	filename  :
	[
		/registra.tio/i, /descript.ion/i,
		/file_id.diz/i,
		/^disk_ord.er.?$/i, /ordrform/i,
		/^(about|whatsnew|copying|changes|todo|problems|description|readme|readthis|readnow|order|manual|release)[._-]*($|\..+$)/i,
		/^.*read\..*me.*$/i, /^.*read.*me.*\./i, /^.*read.?me.?$/i, /^read.*me.*$/i, /^.read_this/i,
		/^.*manu.al$/i
	]
};

exports.inputMeta = (state, p, cb) => p.family.supportedInputMeta(state, p, cb);
