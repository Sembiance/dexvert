...
// Will delete any files it finds under baseDirPath that are of zero size
exports.deleteEmptyFiles = function deleteEmptyFiles(baseDirPath)
{
	return (state, p, cb) =>
	{
		tiptoe(
			function findFiles()
			{
				fileUtil.glob(baseDirPath, "**", {nodir : true}, this);
			},
			function statFiles(filePaths)
			{
				if(!filePaths || filePaths.length===0)
					return this.finish();
				
				this.data.filePaths = filePaths;
				filePaths.parallelForEach(fs.stat, this);
			},
			function performDeletions(fileStats)
			{
				fileStats.parallelForEach((fileStat, subcb, i) =>
				{
					if(fileStat.size===0)
						fileUtil.unlink(this.data.filePaths[i], subcb);
					else
						setImmediate(subcb);
				}, this);
			},
...
// Will delete any files it finds under baseDirPath that are identical to srcFilePath
exports.deleteDuplicateFiles = function deleteDuplicateFiles(baseDirPath, srcFilePath)
{
	return (state, p, cb) =>
	{
		tiptoe(
			...
			function statFiles(filePaths)
			{
				...
				this.data.filePaths = filePaths;
				hashUtil.hashFile("sha1", srcFilePath, this.parallel());
				filePaths.parallelForEach((filePath, subcb) => hashUtil.hashFile("sha1", filePath, subcb), this.parallel());
			},
			function performDeletions(srcFileHash, fileHashes)
			{
				fileHashes.parallelForEach((fileHash, subcb, i) =>
				{
					if(fileHash===srcFileHash)
						fileUtil.unlink(this.data.filePaths[i], subcb);
					else
						setImmediate(subcb);
				}, this);
