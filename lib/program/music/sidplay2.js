...
exports.args = (state, p, inPath=state.input.filePath) =>
{
...
exports.post = (state, p, cb) => p.util.file.move(path.join(state.output.absolute, `outfile_${state.sidPaddedSubTune}.wav`), path.join(state.output.absolute, `${state.input.name}_${state.sidPaddedSubTune}.wav`))(state, p, cb);
