...
exports.bin = () => "ffmpeg";
exports.args = (state, p, r, inPath=state.input.filePath, outPath=path.join(state.output.dirPath, `outfile${state.ffmpegExt || (state.id.family==="image" ? ".png" : (state.id.family==="audio" ? ".flac" : ".mp4"))}`)) =>
{
	const ffmpegArgs = [];

	// Can run `ffmpeg -formats` for a list
	state.ffmpegExt = state.ffmpegExt || (state.id.family==="image" ? ".png" : (state.id.family==="audio" ? ".flac" : ".mp4"));
	if(state.ffmpegFormat)
		ffmpegArgs.push("-f", state.ffmpegFormat);

	if(state.ffmpegExt===".png")
		ffmpegArgs.push("-i", inPath, "-frames:v", "1", outPath);
	else if(state.ffmpegExt===".flac")
...
{
	const ffmpegExt = state.ffmpegExt || ".mp4";
	delete state.ffmpegExt;
	p.util.file.move(path.join(state.output.absolute, `outfile${ffmpegExt}`), path.join(state.output.absolute, state.input.name + (ffmpegExt)))(state, p, cb);
};
