...
const argv = cmdUtil.cmdInit({
	cmdid   : "dexserver-tridServer",
	version : "1.0.0",
	desc    : "Runs and monitors the tridServer daemon which is used to help identify files",
	opts    :
	{
...
const exitHandler = async ({success, code, signal}) =>
{
	...
	xlog.error`tridServer exited unexpectedly (success: ${success}) with code: ${code}, signal: ${signal}`;
	await start();
	xlog.info`tridServer restarted successfully`;
};

start = async () =>
{
	xlog.info`Starting tridServer...`;
	({p} = await runUtil.run("python3", ["tridServer.py", `--port=${C.TRID_SERVER_PORT}`, `--workers=${C.IS_DEV_MACHINE ? 3 : 15}`], {cwd : Program.binPath("tridServer"), detached : true, exitcb : async o => await exitHandler(o), stdoutcb : line => xlog.info`${line}`, stderrcb : line => xlog.warn`${line}`}));
	await xu.waitUntil(async () => (await xu.fetch(`http://${C.TRID_SERVER_HOST}:${C.TRID_SERVER_PORT}/status`, {silent : true}))?.trim()==="a-ok");
};
