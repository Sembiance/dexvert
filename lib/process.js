...
exports.steps = function steps(state, p)
{
	return [
		...
		p.util.flow.batchRepeatUntil([
			exports.processNext,
			p.util.file.deleteEmptyFiles(state.output.absolute),
			p.util.file.deleteDuplicateFiles(state.output.absolute, state.input.absolute),
...
exports.processNext = function processNext(state, p, cb)
{
	...
	{
		if(state.id.brute && p.program[format.meta.program].meta.bruteUnsafe)
