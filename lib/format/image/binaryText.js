...
exports.converterPriorty = ["deark", {program : "ansilove", stateFlags : {ansiloveType : "bin"}}, "abydosconvert", "ffmpeg"];
...
exports.inputMeta = (state, p, cb) =>
{
	tiptoe(
		...
		{
			if(state.run.meta.ffprobe)
			{
				state.input.meta.binaryText = state.run.meta.ffprobe;
				delete state.run.meta.ffprobe;
			}
