...
exports.steps =
[
	// abydosconvert handles IFF files the best, including color cycling animations in WEBP format (AH_Dan, AH_Eye, Watch, DECKER-BattleMech). But it does have issues with foto57 as of v0.2.3
	// So we only keep abydosconvert output if it produces an animated .webp image
	// Otherwise we fall back to recoil2png which produces better PNG image results
	// nconvert and convert are not used as they both mess up these: beautyfc, breakfst, hameset*, pchgcmp, agony
	() => ({program : "abydosconvert"}),
	(state, p) => p.util.file.findValidOutputFiles(),
	(state, p) => p.family.validateOutputFiles,
	(state, p) => ((state.output.files || []).some(v => v.toLowerCase().endsWith(".webp")) ? p.util.flow.noop : ({program : "recoil2png"}))
];
