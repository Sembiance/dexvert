...
[
	() => (state, p, cb) =>
	{
			...
			function checkForNullSampleRate(inputBuffer)
			{
				...
				if(vhdrLoc!==-1)
				{
					...
					if(inputBuffer.readUInt16BE(offsetLoc)===0)
					{
						...
						[8000, 11025, 16000, 22050, 32000, 37800, 44100, 48000].parallelForEach((sampleRate, subcb) => convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, subcb), this);
						return;
					}
				}
		...
		function convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, convertcb, tries=3)
		{
			...
			tiptoe(
				...
				function retryIfNeeded(outputExists)
				{
					if(outputExists)
						...
						convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, this, tries-1);
					else
						this();
				},
				convertcb
