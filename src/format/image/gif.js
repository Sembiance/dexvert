...
export class gif extends Format
{
	...
	ext          = [".gif"];
	mimeType     = "image/gif";
	magic        = ["GIF image data", "GIF animated bitmap", "Mac PageMill's GIF bitmap (MacBinary)", "Macintosh GIF", /^GIF8[79]a bitmap$/, /^fmt\/(3|4)( |$)/];
	...
	converters = dexState =>
	{
		...
		if(dexState.hasMagics("Mac PageMill's GIF bitmap (MacBinary)"))
			r.push("deark[mac][deleteADF][convertAsExt:.gif]");
		r.push("nconvert", "deark[module:gif]", "hiJaakExpress", "picturePublisher", "corelPhotoPaint", "canvas", "tomsViewer", "corelDRAW");
		return r;
	};
}
