...
{
	state.sidPaddedSubTune = `${(r.flags.sidSubTune || 1)}`.padStart(3, "0");
	const sidplayArgs = [`-w${path.join(state.output.dirPath, `outfile_${state.sidPaddedSubTune}.wav`)}`, `-o${r.flags.sidSubTune || 1}`];
