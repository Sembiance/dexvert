...
function setFallthroughID(state)
{
	...
	if(unsupportedIdentifications.length>=1)
	{
		...
		state.id = unsupportedIdentifications.multiSort([id => id.matchType==="ext", id => id.confidence, id => C.FAMILIES.indexOf(id.family)], [true, false])[0];
	}
}
...
exports.processNext = function processNext(state, p, cb)
{
	...
	function cbHandler(err)
	{
		if(err)
		{
			console.error(`Encountered error attempting to process as format: ${state.id.family}/${state.id.formatid}`);
			if(state.verbose>=1)
				console.log(state);
			console.error(err);
		}
