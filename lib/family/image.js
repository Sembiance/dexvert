...
[
	(state, p) => p.util.dos.run({state, p, bin : "ALCHEMY.EXE", autoExec : [`ALCHEMY.EXE -t ${state.input.filePath} OUT.TIF`]}),
	...
	(state.output.files || []).slice().parallelForEach((outSubPath, subcb) =>
	{
		...
		tiptoe(
			...
			function classifyImage(outIdentifications)
			{
				...
				imageUtil.getInfo(outFilePath, {simpleOnly : true}, this.parallel());

				if(["gif", "png", "jpg", "webp"].includes(dexid.formatid))
					httpUtil.post(`http://${C.TENSORSERV_HOST}:${C.TENSORSERV_PORT}/classify/garbage`, {imagePath : outFilePath}, {postAsJSON : true}, this.parallel());
				else
					this.parallel()(undefined, [null]);
			},
			function checkImageValidity(imageInfo, [gpRaw])
			{
				...
				if(gpRaw)
				{
					...
					if(gp!==null && gp>0 && gp<1)
					{
						if(!state.output.hasOwnProperty("garbageProbability"))
							state.output.garbageProbability = {};
						state.output.garbageProbability[`${state.id.family}/${state.id.formatid}:${outSubPath}`] = gp;
					}

					if(gp===null || gp>C.MATCH_MAX_GARBAGE_PROBABILITIES[state.id.matchType])
					{
						if(gp===null)
							throw new Error(`Failed to calculate garbage probability for: ${outFilePath} with result: ${gpRaw.toString()}`);
						else
							fs.copyFileSync(outFilePath, path.join(__dirname, "../../tensor/garbage/detected", `${gp.noExponents()}-${Math.randomInt(1, 100000)}-${Date.now().toString()}-${state.id.formatid}-${state.input.absolute.replaceAll("/", ":")}-${outSubPath}`));
