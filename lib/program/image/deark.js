	...
	else
	{
		tiptoe(
			...
			function getShaSums(outputFilePaths)
			{
				...
				this.data.outputFilePaths.parallelForEach((outputFilePath, subcb) => hashUtil.hashFile("sha1", outputFilePath, subcb), this);
			},
			function convertOutputFiles(shaSums)
			{
				...
				this.data.outputFilePaths.parallelForEach((outputFilePath, subcb, i) =>
				{
					if(r.flags.dearkRemoveDups)
					{
						...
						const shaSum = shaSums[i];
						if(!seenSums.includes(shaSum))
							seenSums.push(shaSum);
						else
