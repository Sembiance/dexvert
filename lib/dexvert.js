...
const XU = require("@sembiance/xu"),
	...
	{validateProgram} = require(path.join(__dirname, "validate.js")),
...
function init(state, cb)
{
	...
	tiptoe(
		function findUtils()
		{
			...
			dexUtil.findFormats(state, this.parallel());
		},
		function findFiles(utilFilePaths, programFilePaths, familyFilePaths, formats)
		{
			...
			[["util", utilFilePaths], ["program", programFilePaths], ["families", familyFilePaths]].forEach(([type, filePaths]) => filePaths.forEach(filePath =>
			{
				...
				if(state.verbose>=2 && type==="program")
					validateProgram(o);
...
exports.process = function process(inputFilePath, outputDirPath, {tmpDirPath=os.tmpdir(), verbose=0, brute=false, keepGoing=false, alwaysBrute=false, brutePrograms=false, midiFont=null}, cb)
{
