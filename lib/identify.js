...
[
	...
	(state0, p0) => p0.util.flow.parallel([
		...
		state => ({program : "file", argsd : [state.identifyTrimmedFilePath || state.input.filePath]}),
		state => ({program : "trid", argsd : [state.identifyTrimmedFilePath || state.input.filePath]})]),
	(state0, p0) => (state0.identifyTrimmedFilePath ? p0.util.file.unlink(state0.identifyTrimmedFilePath) : p0.util.flow.noop),
	() => exports.identify
];

// Some text files in DOS/Windows end with a single 0x1A character, signifying the end of the file
// Sadly file/trid get confused by this and just view the file as 'data'
// So my self-made stripEOF1a program checks to see if it ends in 0x1A and if so it will copy the file over, sans that last byte
// Note if ANY characters in the file are not printable (\t\r\n included) then it aborts
exports.trimEOFChar = function trimEOFChar(state, p, cb)
{
	state.identifyTrimmedFilePath = fileUtil.generateTempFilePath(state.tmpDirPath, state.input.ext);
	tiptoe(
		function performTrim()
		{
			runUtil.run(path.join(__dirname, "..", "bin", "stripEOF1A"), [state.input.absolute, state.identifyTrimmedFilePath], {silent : true}, this);
		},
		function checkResult()
		{
			if(!fileUtil.existsSync(state.identifyTrimmedFilePath))
				delete state.identifyTrimmedFilePath;
...
exports.identify = function identify(state, {C, formats}, cb)
{
	...
	Object.forEach(formats, (formatFamily, formatTypes) =>
	{
		...
		Object.forEach(formatTypes, (formatid, format) =>
		{
			...
			["encoding", "confidenceAdjust", "website", "mimeType", "hljsLang"].forEach(key =>
			{
				if(meta[key])
