...
export class rawPartition extends Format
{
	...
	magic          = [
		...
		"Linux LVM2 volume", /^LVM2 PV/, /^ATARI ST partition map.*GEMDOS.*FAT16/,
	...
	converters       = dexState =>
	{
		...
		converters.push("aaru");
		if((dexState.meta?.partitions?.[0]?.start || 0)>0)
			converters.push(`uniso[offset:${dexState.meta.partitions[0].start}]`);
