...
exports.args = state =>
{
	...
	{
		state.ffmpegExt = ".png";
		return ["-i", state.input.filePath, "-frames:v", "1", path.join(state.output.dirPath, `outfile${state.ffmpegExt}`)];
	}

	// Assume video
	state.ffmpegExt = ".mp4";
	return ["-i", state.input.filePath, "-c:v", "libx264rgb", "-crf", "0", "-preset", "ultrafast", path.join(state.output.dirPath, `outfile${state.ffmpegExt}`)];
};
