...
exports.processNext = function processNext(state, p, cb)
{
	...
	p.util.flow.serial([
		...
		subState =>
		{
			const ext = p.format.meta.safeExt ? p.format.meta.safeExt(state) : (p.format.meta.ext ? (p.format.meta.ext.includes(subState.input.ext.toLowerCase()) ? subState.input.ext : p.format.meta.ext[0]) : (subState.input.ext || ""));
		...
		() => (ss, sp, scb) => ["filesRequired", "filesOptional"].flatMap(t => ((p.format.meta[t] || (() => []))(ss, ss.input.otherFiles) || [])).unique().parallelForEach((v, vcb) => fs.symlink(path.join(ss.input.dirPath, v), path.join(ss.cwd, `in${path.extname(v).toLowerCase()}`), vcb), scb),	// eslint-disable-line max-len
		() => p.util.meta.input,
		...(p.format.preSteps || []),
