...
[
	(state, p) =>
	{
		...
		if(!(p.format.converterExclude || []).includes("*"))
			state.converters.push(...DEFAULT_CONVERTERS.filter(d => !(p.format.converterExclude || []).includes(d) && !state.converters.some(c => c.program===d) && !p.program[d].meta.unsafe).map(d => ({program : d})));
