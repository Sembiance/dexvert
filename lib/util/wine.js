...
exports.winePath = function winePath(unixPath)
{
...
exports.run = function run({cmd, args, cwd, delay=XU.SECOND*5, timeout=XU.MINUTE*2, recordVideo, autoItScript, preWineCleanup, isolate, selfLock})
{
	return (state, p, cb) =>
	{
		...
		const winePrefixDirPath = isolate ? fileUtil.generateTempFilePath() : C.WINE_PREFIX_DIR_PATH;
		...
		tiptoe(
			...
			function extractEnv()
			{
				runUtil.run("tar", ["-xf", path.join(WINE_DIR_PATH, "env.tar"), "-C", winePrefixDirPath], runUtil.SILENT, this.parallel());
			},
			function waitForLock()
			{
				if(selfLock)
					dexUtil.waitForLock(cmd, this);
				else
					this();
			},
			function launchWine()
			{
				...
				const cmdFilePath = cmd.startsWith("C:") ? path.join(winePrefixDirPath, "drive_c", cmd.substring(2)) : path.join(WINE_DIR_PATH, cmd);
				this.data.wineCP = runUtil.run("wine", [cmdFilePath, ...Array.force(args)], runArgs);
			...
			function cleanup()
			{
				...
				if(isolate && state.verbose<4)
					fileUtil.unlink(winePrefixDirPath, this.parallel());

				if(selfLock)
					dexUtil.releaseLock(cmd, this.parallel());
			},
