...
async function extractNormalISO()
{
	...
	// the --inplace arg allows it to copy files out of corrupt CD ISOs such as the .exe from: https://discmaster.textfiles.com/admin/item?itemid=38528
	await runUtil.run("sudo", ["rsync", "-sa", "--inplace", `${MOUNT_DIR_PATH}/`, `${OUT_DIR_PATH}/`], {liveOutput : xlog.atLeast("debug")});
...
async function extractHFSISO()
{
	...
	const recurseHFS = async function recurseHFS(subPath)
	{
		...
		// copy out files
		const hlsRegex = /^\s*(?<id>\d+)\s+(?<type>[Fdf])[i\s]\s(?<fileType>.{4})?\/?(?<fileCreator>.{4})?.+(?<month>[A-Z][a-z]{2})\s+(?<day>[\s\d]\d)\s+(?<yearOrTime>\d\d:\d\d|\d{4})\s?(?<filename>.+)$/;
