...
[
	() => (state, p, cb) =>
	{
			...
			function checkForNullSampleRate(inputBuffer)
			{
				...
				if(vhdrLoc!==-1)
				{
					...
					if(inputBuffer.readUInt16BE(offsetLoc)===0)
					{
						...
						[8000, 11025, 16000, 22050, 32000, 37800, 44100, 48000].serialForEach((sampleRate, subcb) => convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, subcb), this);
						return;
					}
				}
		...
		function convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, convertcb, tries=10)
		{
			const tmpFilePath = fileUtil.generateTempFilePath(state.tmpDirPath, ".8svx");
			const bufferData = Buffer.from(inputBuffer);
			...
			tiptoe(
				...
				{
					fileUtil.exists(tmpFilePath, this.parallel());
				},
				function retryIfNeeded(outputExists)
				{
					if(outputExists)
						fileUtil.unlink(tmpFilePath, this);
					else if(tries>0)
						setTimeout(() => convertWithSampleRate(inputBuffer, offsetLoc, sampleRate, this, tries-1), Math.randomInt(100, 2000));
					else
						this();
				},
				convertcb
