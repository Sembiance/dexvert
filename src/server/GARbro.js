...
const argv = cmdUtil.cmdInit({
	cmdid   : "dexserver-GARbro",
	version : "1.0.0",
	desc    : "Runs and monitors the GARbro.Server daemon which is used to help identify and extract files",
	opts    :
	{
...
const exitHandler = async ({success, code, signal}) =>
{
	...
	xlog.error`GARbro server exited unexpectedly (success: ${success}) with code: ${code}, signal: ${signal}`;
	await start();
	xlog.info`GARbro server restarted successfully`;
};
