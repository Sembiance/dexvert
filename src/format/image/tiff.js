...
export class tiff extends Format
{
	...
	magic        = [
		// Generic TIFF files
		"Tagged Image File Format", "TIFF image data", "TIFF Datei", "Macintosh TIFF bitmap (MacBinary)", "image/tiff", "piped tiff sequence (tiff_pipe)", "eXtended Image File Format bitmap", /^deark: tiff \((TIFF|XIFF)\)/, /^(Geo)?TIFF (\((jpeg|Tiled)\) )?:(cpt|stw|tiff):$/,
		/^fmt\/(154|155|353)( |$)/, /^x-fmt\/(387|388|399)( |$)/,

		// App-Specific TIFF files
		"CorelChart chart", "QuickLink II Fax bitmap (new)", "Hasselblad 3F RAW image", "Wang Image File Format bitmap (TIFF with WANG tags)", /^fmt\/(1312|1313|1480|2091)( |$)/
	];
	...
	converters = dexState =>
	{
		...
		if(!dexState.hasMagics(_MACBINARY_MAGIC))
			r.push("imageAlchemy", "graphicWorkshopProfessional", "photoDraw", "hiJaakExpress", "imjview", "gimp", "canvas", "tomsViewer", "picturePublisher", "corelDRAW", "keyViewPro", "pv[matchType:magic]");
		return r;
	};
}
