...
export class seattleFilmWorks extends Format
{
	...
	metaProvider = ["image"];
	converters   = ["convert", "nconvert[format:sfw]", "gimp[matchType:magic]", `abydosconvert[format:${this.mimeType}]`, "nconvert[format:pmp]"];	// the pmp is for a few that don't properly convert to sfw but are indeed SFW files
}
