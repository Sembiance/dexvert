...
export class wav extends Format
{
	...
	magic        = [
		...
		/^Generic RIFF file WAVE$/, "deark: riff (WAVE)", /^geViewer: WAV_RIFF( |$)/,
		/^fmt\/(2|6|141|142|143|703|704|706|709)( |$)/
	];
	idMeta       = ({macFileType}) => [".WAV", "WAVE"].includes(macFileType);
	metaProvider = ["soxi"];
	converters   = ["sox[type:wav]", "ffmpeg[format:wav][outType:mp3]", "awaveStudio[strongMatch]"];
}
