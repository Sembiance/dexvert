...
exports.processNext = function processNext(state, p, cb)
{
	...
	p.util.flow.serial([
		...
		subState => p.util.file.safeInput(p.format.meta.safeExt ? p.format.meta.safeExt(state) : (p.format.meta.ext ? (p.format.meta.ext.includes(subState.input.ext.toLowerCase()) ? subState.input.ext : p.format.meta.ext[0]) : (subState.input.ext || ""))),
