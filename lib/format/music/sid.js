...
exports.inputMeta = (state, p, cb) =>
{
	tiptoe(
		function getSIDInfo()
		{
			runUtil.run("sidplay2", ["-w/dev/null", "-t1", state.input.filePath], {silent : state.verbose<=1, liveOutput : state.verbose>=5, timeout : XU.MINUTE*5, cwd : state.cwd}, this.parallel());
