	...
	(state, p) =>
	{
		...
		state.converters.filterInPlace(cp =>
		{
			if(Object.isObject(cp))
			{
				...
				if(cp.unsafe && !excplictlyIncluded)
					return false;
			}
...
exports.ansiArtInputMeta = function ansiArtInputMeta(state, p, cb)
{
	tiptoe(
		...
		{
			p.util.program.run(p.program.deark, {args : ["-opt", "char:output=html", "-od", state.cwd, "-o", state.input.name, state.input.filePath]})(state, p, this);
		},
