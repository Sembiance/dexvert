...
[
	() => (state, p, cb) =>
	{
			...
			function checkForNullSampleRate(inputBuffer)
			{
				...
				if(vhdrLoc!==-1)
				{
					...
					if(inputBuffer.readUInt16BE(offsetLoc)===0)
					{
						...
						this.data.tmpFilePath = fileUtil.generateTempFilePath(undefined, ".8svx");
						[8000, 11025, 16000, 22050, 32000, 37800, 44100, 48000].serialForEach((sampleRate, subcb) => convertWithSampleRate(this.data.tmpFilePath, inputBuffer, offsetLoc, sampleRate, subcb), this);
						return;
					}
				}
		...
		function convertWithSampleRate(tmpFilePath, inputBuffer, offsetLoc, sampleRate, convertcb)
		{
			tiptoe(
				function writeTmpFile()
				{
					inputBuffer.writeUInt16BE(sampleRate, offsetLoc);
					fs.writeFile(tmpFilePath, inputBuffer, this);
				},
