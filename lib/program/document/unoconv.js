...
exports.bin = () => "unoconv";
exports.args = (state, p, inPath=state.input.filePath, outPath=path.join(state.output.dirPath, `outfile.${(state.unoconvType || (state.id.family==="image" ? "png" : "pdf"))}`)) => [
	"-n", "-p", `${C.UNOCONV_PORT}`, "-f", (state.unoconvType || (state.id.family==="image" ? "png" : "pdf")), "-o", outPath, inPath];

exports.post = (state, p, cb) => p.util.file.move(path.join(state.output.absolute, `outfile.${(state.unoconvType || (state.id.family==="image" ? "png" : "pdf"))}`),
