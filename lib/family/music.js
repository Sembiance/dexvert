	...
	(state.output.files || []).slice().parallelForEach((outSubPath, subcb) =>
	{
		...
		tiptoe(
			...
			function convertOrRemove()
			{
				...
				p.util.program.run("ffmpeg", {flags : {ffmpegExt : ".flac"}, argsd : [outFilePath, path.join(path.dirname(outFilePath), `${path.basename(outFilePath, path.extname(outFilePath))}.flac`)]})(state, p, this);
			},
