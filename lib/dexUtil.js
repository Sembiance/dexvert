		...
		function loadFormats(familyFormatPaths)
		{
			...
			C.FAMILIES.forEach((FAMILY, i) =>
			{
				familyFormatPaths[i].forEach(familyFormatPath =>
				{
					...
					const meta = Object.fromEntries(Object.entries(format.meta));

					const unknownMetaKeys = Object.keys(format.meta).subtractAll(["name", "website", "ext", "safeExt", "mimeType", "forbidExtMatch", "unsupported", "magic", "forbiddenMagic", "filename", "priority", "weakMagic", "trustMagic",
						"notes", "bruteUnsafe", "filesize", "keepFilename", "symlinkUnsafe", "encoding", "confidenceAdjust", "forbidFilesizeMatch", "filesRequired", "filesOptional"]);
					if(unknownMetaKeys.length>0)
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has extra meta keys: ${unknownMetaKeys}`);

					const unknownFormatKeys = Object.keys(format).subtractAll(["meta", "steps", "post", "updateProcessed", "idCheck", "inputMeta", "preSteps", "postSteps", "converterPriorty", "converterExclude"]);
					if(unknownFormatKeys.length>0)
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has extra keys: ${unknownFormatKeys}`);

					meta.formatid = formatid;
					meta.family = FAMILY;

					if((meta.ext || []).some(ext => ext.match(/[A-Z]/)))
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has a meta.ext that is not lowercase!`);

					if(meta.filesize && !Object.isObject(meta.filesize) && !Array.isArray(meta.filesize) && typeof meta.filesize!=="number")
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has an invalid meta.filesize type. Must be a number, array of numbers or an object with ext key and number/array of number values`);

					if(meta.filesize && Object.isObject(meta.filesize) && !(meta.ext || []).includesAll(Object.keys(meta.filesize).flatMap(k => k.split(","))))
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} has a meta.filesize ext that is not in meta.ext`);

					if(!meta.hasOwnProperty("name"))
						process.exit(XU.log`dexUtil.js: Format ${FAMILY}/${formatid} does not have a meta.name!`);
