	...
	return (state, p, cb) =>
	{
		...
		tiptoe(
			...
			function preArgs()
			{
				...
				program.preArgs(state, p, this);
			},
			function generateArgs()
			{
				state.run.args[path.basename(bin)] = options.args || (options.argsd ? program.args(state, p, ...options.argsd) : program.args(state, p));
			...
			function executeBin()
			{
				...
				if(program.wine)
				{
					const wineOpts = {cmd : bin, args : state.run.args[path.basename(bin)]};
					if(program.cwd)
				...
				{
					runUtil.run(bin, state.run.args[path.basename(bin)], runOptions, this.parallel());
				}
			},
