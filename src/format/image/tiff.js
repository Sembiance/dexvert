...
export class tiff extends Format
{
	...
	magic        = [
		// Generic TIFF files
		"Tagged Image File Format", "TIFF image data", "Macintosh TIFF bitmap (MacBinary)", "image/tiff", "piped tiff sequence (tiff_pipe)", "deark: tiff (TIFF)", /^TIFF (\((jpeg|Tiled)\) )?:tiff:$/, /^fmt\/(154|155|353)( |$)/, /^x-fmt\/(387|388|399)( |$)/,

		// App-Specific TIFF files
		"CorelChart chart", "QuickLink II Fax bitmap (new)", "Hasselblad 3F RAW image", /^fmt\/(1312|1313)( |$)/
	];
	...
	converters = dexState =>
	{
		...
		r.push("deark[module:tiff][noThumbs]", "iconvert", "convert", "iio2png", "wuimg");	// iconvert handles ycbcr-cat.tif properly
