...
const qty = maxQty => (DEBUG ? 1 : (Math.min(maxQty, (Deno.hostname().startsWith("slim") ? maxQty*0.5 : ({lostcrag : 3, crystalsummit : 2}[Deno.hostname()] || maxQty)))));
const OS =
{
	...
	amigappc : { qty :  qty(5), ram : "1G", arch :    "ppc", inOutType :  "http", scriptExt : ".rexx", cores : 1, machine : "type=sam460ex", net : "ne2k_pci", hdOpts : ",id=disk", extraArgs : ["-device", "ide-hd,drive=disk,bus=ide.0"]},
