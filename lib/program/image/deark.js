...
exports.bin = () => "deark";
exports.args = (state, p, inPath=state.input.filePath, outPath=state.output.dirPath, outName=state.input.name) => (["-opt", `char:output=${state.dearkCharOutput || "image"}`, "-od", outPath, "-o", outName, inPath]);
	...
	else
	{
		tiptoe(
			...
			function convertOutputFiles(outputFilePaths)
			{
				...
				outputFilePaths.parallelForEach((outputFilePath, subcb) =>
				{
					...
					if(ext===".qtif")
						...
						p.util.program.run("recoil2png", {args : p.program.recoil2png.args(state, p, path.join(state.output.dirPath, path.basename(outputFilePath)), path.join(state.output.absolute, `${path.basename(outputFilePath, ext)}.png`))})(state, p, subcb);
					else
						p.util.program.run("convert", {args : p.program.convert.args(state, p, path.join(state.output.dirPath, path.basename(outputFilePath)), path.join(state.output.dirPath, `${path.basename(outputFilePath, ext)}.png`))})(state, p, subcb);
				}, this);
			},
			...
			function renameIfNeeded(outputFilePaths)
			{
				...
				if(state.dearkJoinFrames)
				{
					this.data.GIFFilePath = path.join(state.output.absolute, `${state.input.name}.gif`);
					p.util.program.run("convert", {args : ["-delay", "12", "-loop", "0", "-dispose", "previous", ...outputFilePaths, ...p.program.convert.args(state, p).slice(1, -1), this.data.GIFFilePath]})(state, p, this);
				}
				else
				{
			...
			{
				state.ffmpegExt = ".mp4";
				p.util.program.run("ffmpeg", {args : p.program.ffmpeg.args(state, p, this.data.GIFFilePath)})(state, p, this);
			},
