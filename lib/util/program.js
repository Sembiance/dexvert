...
exports.run = function run(programRaw, options={})
{
	return (state, p, cb) =>
	{
		...
		const bin = program.steps ? null : (program.qemu || program.bin)(state);
		...
		tiptoe(
			...
			function executeBin()
			{
				...
				if(program.qemu)
				{
					r.qemuOptions = {cmd : bin, args : r.args};
					if(program.qemuOptions)
						Object.assign(r.qemuOptions, program.qemuOptions(state, p, r));

					p.util.qemu.run(r.qemuOptions)(state, p, runcb);
				}
				else
				{
