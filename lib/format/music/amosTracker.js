...
[
	() => (state, p, cb) =>
	{
		tiptoe(
			...
			{
				p.util.program.run("xmp", {args : p.program.xmp.args(state, p, this.data.modFilename)})(state, p, this);
			},
			function getMusicInfo()
			{
				p.util.program.run("modInfo", {args : p.program.modInfo.args(state, p, path.join(state.cwd, this.data.modFilename))})(state, p, this);
			},
