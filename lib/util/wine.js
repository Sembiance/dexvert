...
exports.run = function run({cmd, args, cwd, delay=XU.SECOND*5, timeout=XU.MINUTE*10, autoItScript})
{
	return (state, p, cb) =>
	{
		...
		tiptoe(
			...
			function launchWine()
			{
				const runArgs = {detached : true, virtualX : true, silent : state.verbose<=1, liveOutput : state.verbose>=3, virtualXPortNumFile : portNumFilePath, timeout, env : {WINEPREFIX : winePrefixDirPath}};
				if(cwd)
					runArgs.cwd = cwd;
				if(state.verbose>=4)
				{
