...
{
	return [
		p.util.program.run(p.program.undirector, {args : ["pc", state.input.filePath, state.output.dirPath]}),
		p.util.program.run(p.program.undirector, {args : ["mac", state.input.filePath, state.output.dirPath]})
	];
};
