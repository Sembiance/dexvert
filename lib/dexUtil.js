			...
			C.FAMILIES.forEach((FAMILY, i) =>
			{
				familyFormatPaths[i].forEach(familyFormatPath =>
				{
					...
					const unknownMetaKeys = Object.keys(format.meta).subtractAll(["name", "website", "ext", "safeExt", "mimeType", "forbidExtMatch", "unsupported", "magic", "forbiddenMagic", "filename", "priority", "weakMagic", "trustMagic",
						"notes", "bruteUnsafe", "filesize", "keepFilename", "symlinkUnsafe"]);
					if(unknownMetaKeys.length>0)
						process.exit(XU.log`Format ${FAMILY}/${formatid} has extra meta keys: ${unknownMetaKeys}`);
					...
					if(unknownFormatKeys.length>0)
						process.exit(XU.log`Format ${FAMILY}/${formatid} has extra keys: ${unknownFormatKeys}`);

					meta.formatid = formatid;
					meta.family = FAMILY;

					if((meta.ext || []).some(ext => ext.match(/[A-Z]/)))
						process.exit(XU.log`Format ${FAMILY}/${formatid} has an ext that is not lowercase!`);

					if(!meta.hasOwnProperty("name"))
						process.exit(XU.log`Format ${FAMILY}/${formatid} does not have a name!`);
