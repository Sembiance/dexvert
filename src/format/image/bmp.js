...
export class bmp extends Format
{
	...
	magic          = [
		...
		"Alpha Microsystems Bitmap", "image/x-dib", "piped bmp sequence (bmp_pipe)", "PC bitmap", /Bitmap Bild \(Typ \d/, /^fmt\/(114|115|116|117|118|119)( |$)/, /^x-fmt\/270( |$)/,
	...
	converters   = dexState =>
	{
		...
		r.push(...["canvas5", "graphicWorkshopProfessional", "photoDraw", "paintDotNet", "keyViewPro", "imageAlchemy", "hiJaakExpress", "corelPhotoPaint", "canvas[matchType:magic][hasExtMatch]", "tomsViewer", "pv"].map(v => `${v}[matchType:magic][strongMatch]`));
		return r;
	};
}
