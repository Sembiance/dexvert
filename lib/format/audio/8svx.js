	...
	() => (state, p, cb) =>
	{
			...
			function checkForNullSampleRate(inputBuffer)
			{
				...
				if(vhdrLoc!==-1)
				{
					...
					if(inputBuffer.readUInt16BE(offsetLoc)===0)
					{
						...
						return [8000, 11025, 16000, 22050, 32000, 37800, 44100, 48000].serialForEach((sampleRate, subcb) => convertWithSampleRate(tmpFilePath, inputBuffer, offsetLoc, sampleRate, subcb), this);
					}
				}

				// Fallback
				p.util.program.run(p.program.ffmpeg)(state, p, this);
			},
			cb
		);
		...
		function convertWithSampleRate(tmpFilePath, bufferData, offsetLoc, sampleRate, convertcb)
		{
			tiptoe(
				...
				{
					p.util.program.run(p.program.ffmpeg, {args : ["-i", tmpFilePath, path.join(state.output.dirPath, `${state.input.name}_${sampleRate}.wav`)]})(state, p, this);
				},
				function remoteTmpFile()
				{
