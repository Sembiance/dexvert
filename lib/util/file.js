	...
	return (state, p, cb) =>
	{
		...
		tiptoe(
			...
			function gatherOutputFileInfo(outputFilePaths)
			{
				...
				if(state.verbose>=5)
					XU.log`file.findValidOutputFiles found ${state.output.files.length} output files.`;
				...
				hashUtil.hashFile("blake3", state.input.absolute, this.parallel());
				outputFilePaths.parallelForEach((filePath, subcb) => hashUtil.hashFile("blake3", filePath, subcb), this.parallel());
