 ...
 */
int getblock(void *vp, unsigned short size, unsigned char *e)
{
    int c;
    char eof;
...
unsigned short Tracks,			// Total # of tracks processed
...
unsigned char *Dseg;  // Sector data segment
unsigned short Dtop;			// Segment top
unsigned short Doffset[256];	// Data offset
 ...
 */
unsigned char *txt(unsigned short v, unsigned char *t[], unsigned short s)
{
 ...
 */
void dump(void *vp, unsigned char *t, unsigned char Mask)
{
    unsigned char c;
    unsigned char *p = (unsigned char *)vp;
    ...
    if(Debug & Mask)
        ...
        case 0x81 : Dprintf("%04X", *(unsigned short *)p); p += 2;	continue;
        case 0x82 : Dprintf("%d", *p++);					continue;
        case 0x83 : Dprintf("%d", *(unsigned short *)p);	p += 2;	continue;
        default: Dputc(c); }
}
 ...
 */
unsigned short compute_crc(const void *vp, unsigned short len, unsigned short crc)
{
...
{
    unsigned short c, i, j, nl, crc, len;
    ...
    c = compute_crc(&len, 2, j = nl = 0);
    c = compute_crc(&y, 1, c);
    c = compute_crc(&mo, 1, c);
    c = compute_crc(&d, 1, c);
    c = compute_crc(&h, 1, c);
    c = compute_crc(&mi, 1, c);
    c = compute_crc(&s, 1, c);

    sprintf(Cmap, " %u/%02u/%04u %u:%02u:%02u", d, mo+1, (unsigned short)y+1900, h, mi, s);
    ...
    while(j < len) {				// Process each character
        ++j;
        i = getbyte();
        c = compute_crc((byte *)&i, 1, c);	// Update CRC
 ...
 */
int issame(unsigned char *d, unsigned char v, unsigned short s)
{
    while(s) {
        --s;
        if(*d++ != v)
 ...
 */
void write_data(unsigned short index)
{
    unsigned short t, s, d, p, Size, End;
    unsigned char b;
 ...
 */
unsigned short getnum(unsigned short b)
{
    unsigned short v, c;
    char f;
    v = f = 0;
...
void filename(unsigned char *file, unsigned char *ext, char dropext)
{
    ...
    for(;;) switch(*dest++ = *file++) {
        case 0 :						// End of string
            ...
            return;
        case ':' :						// Disk specifier
        case '\\': p = 0;  ef = 0;	   // Directory specifier
            continue;
...
void main(int argc, char *argv[])
{
    ...
    if(!(Dseg = (unsigned char *)malloc(0x10000)))
        error("Out of memory");
...
//	IOB_size = 4096;
    ...
    while(!getblock(&Thead, sizeof(Thead), 0)) {
        ...
        for(s=0; s < Thead.Tsectors; ++s)
            process_sector();
		Size = Smap[0];
        if(!issame(Smap, Size, Stop))	// 765 cannot do mixed sector sizes
            error("Cannot do mixed sector size within track");
        if(!issame(Cmap, (byte)Cylinder, Stop))		// Cylinder map required
            Head |= 0x80;
