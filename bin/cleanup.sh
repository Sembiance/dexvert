...
if [[ "$whoami" != "sembiance" ]]; then
	echo "Error: This script must be run as 'sembiance', not '$whoami'." >&2
	exit 1
fi
...
killall Xvfb
killall -9 at-spi-bus-launcher helpdeco gimp-org-file-pnm identify convert 86Box timidity gimp x11vnc uadecore ffmpeg
killall wineserver python3 sf java uniconvertor oosplash soffice.bin texi2dvi DirectorCastRipper.exe pdfetex pdflatex tail arj
