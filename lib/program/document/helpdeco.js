...
exports.post = (state, p, cb) =>
{
	tiptoe(
		...
		function convertToPNG()
		{
			...
			{
				if(bitmapFilePath.endsWith(".shg"))
					p.util.program.run(p.program.convert, {args : [`${bitmapFilePath}.000.bmp`, ...p.program.convert.STRIP_ARGS, `${bitmapFilePath}.png`]})(state, p, subcb);
				else
					p.util.program.run(p.program.convert, {args : [bitmapFilePath, ...p.program.convert.STRIP_ARGS, `${bitmapFilePath}.png`]})(state, p, subcb);
			}, this);
		},
		function loadImages()
		{
