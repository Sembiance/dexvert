...
[
	{regex : /{\\field\s+{\\\*\\fldinst\s+import\s+(?<fileName>[^}]+)}}/g},	// windows help standard import
	{regex : /\\{ew[lrc] MVBMP2.*!(?<filename>[^}\\]+)\\}/g}	// multimedia viewer MVBMP2 import
];

const ALLOWED_IMAGE_SIZE_MULTI = 10;	// how many times larger can an extracted image file be versus the original file size
...
// some filenames are standard files which should be never be extracted
const NON_FILENAMES =
[
	// identified from: https://www.oocities.org/mwinterhoff/helpfile.htm
	...
	// others I've found in files, mostly MVB files
	"catalog"
];
		...
		for(const {regex : imageRefRegex} of IMAGE_REF_REGEXES)
		{
			rtfLine = await rtfLine.replaceAsync(imageRefRegex, async (m, filename) =>	// eslint-disable-line no-param-reassign
			{
				...
				if(!(await fileUtil.exists(importFilePath)))
				{
					...
					{
						xlog.error`Import file missing for match ${m}: ${filename} at ${importFilePath}`;
				...
				const importStats = await Deno.stat(importFilePath);
				if(importStats.size===0)
				{
					await fileUtil.unlink(importFilePath);
					xlog.error`Import file ${filename} at ${importFilePath} for match ${m} is a zero bytes sized file, can't convert`;
					return rtfPNG(BROKEN_IMAGE_PNG_HEX);
				}

				if(importStats.size>=(inputFileStats.size*ALLOWED_IMAGE_SIZE_MULTI))
				{
					await fileUtil.unlink(importFilePath);
					xlog.error`Import file ${filename} at ${importFilePath} for match ${m} is more than ${ALLOWED_IMAGE_SIZE_MULTI} times the original input file size, likely a bug in extractions. NOT converting.`;
					return rtfPNG(BROKEN_IMAGE_PNG_HEX);
				}
				...
				{
					xlog.error`Failed to convert import file ${filename} at ${importFilePath} for match ${m} with dexvert`;
					return rtfPNG(BROKEN_IMAGE_PNG_HEX);
				}
...
{
	await Array.from(availableFilenames).parallelMap(async availableFilename =>
	{
		...
		if(extraFileStats.size===0 || extraFileStats.size>(inputFileStats.size*ALLOWED_IMAGE_SIZE_MULTI))
		{
